...
const PricingControl: React.FC = () => {
	...
	const handleSave = (id: number) => {
		const profit_margin =
			((editForm.selling_price - editForm.cost_price) / editForm.cost_price) *
			100;
			...
			<div className="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-4">
				<div>
					...
					<p className="text-neutral-600 mt-1">
						Manage service prices and profit margins
					</p>
				</div>
							...
							{filteredData.map((item) => (
								...
								>
									<td className="px-6 py-4 whitespace-nowrap">
										<div className="flex items-center space-x-3">
											<div
												className={`w-10 h-10 rounded-xl bg-neutral-100 flex items-center justify-center ${item.color}`}
											>
									...
									<td className="px-6 py-4 whitespace-nowrap">
										<span
											className={`text-sm font-medium ${
												item.profit_margin > 3
													? "text-success-600"
													: item.profit_margin > 1
														? "text-warning-600"
														: "text-error-600"
											}`}
										>
